...
class Cell {
 public:
  ...
  explicit Cell(uword n_elem) : n_rows_(n_elem), n_cols_(1), n_slices_(1), data_(n_elem) {}

  Cell(uword rows, uword cols) : n_rows_(rows), n_cols_(cols), n_slices_(1), data_(rows * cols) {}

  Cell(uword rows, uword cols, uword slices)
  ...
  T& operator()(uword idx) {
    DINGO_ASSERT(idx < data_.size(), "dingo::Cell linear index out of range");
    return data_[idx];
  }
  ...
  T& operator()(uword row, uword col, uword slice = 0) {
    DINGO_ASSERT(row < n_rows_ && col < n_cols_ && slice < n_slices_, "dingo::Cell index out of range");
